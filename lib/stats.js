...
export const XP_LEVELS = [
  { level: 1,  minXP: 0,      label: "Novice",      color: "from-slate-400 to-slate-500",     icon: "Sprout" },
  { level: 2,  minXP: 120,    label: "Intern",       color: "from-green-300 to-green-500",     icon: "Briefcase" },
  { level: 3,  minXP: 280,    label: "Trainee",      color: "from-teal-300 to-teal-500",       icon: "GraduationCap" },
  { level: 4,  minXP: 500,    label: "Apprentice",   color: "from-cyan-400 to-cyan-600",       icon: "Wrench" },
  { level: 5,  minXP: 800,    label: "Coder",        color: "from-sky-400 to-blue-500",        icon: "Monitor" },
  { level: 6,  minXP: 1300,   label: "Developer",    color: "from-blue-400 to-indigo-500",     icon: "Zap" },
  { level: 7,  minXP: 2000,   label: "Junior Dev",   color: "from-indigo-400 to-violet-500",   icon: "Rocket" },
  { level: 8,  minXP: 3000,   label: "Mid Dev",      color: "from-violet-400 to-purple-500",   icon: "Target" },
  { level: 9,  minXP: 4500,   label: "Senior Dev",   color: "from-purple-400 to-pink-500",     icon: "Trophy" },
  { level: 10, minXP: 6500,   label: "Tech Lead",    color: "from-pink-400 to-rose-500",       icon: "Crown" },
  { level: 11, minXP: 9000,   label: "Specialist",   color: "from-rose-400 to-red-500",        icon: "Microscope" },
  { level: 12, minXP: 12500,  label: "Architect",    color: "from-orange-400 to-red-500",      icon: "Building2" },
  { level: 13, minXP: 17000,  label: "Expert",       color: "from-amber-400 to-orange-500",    icon: "Star" },
  { level: 14, minXP: 23000,  label: "Principal",    color: "from-yellow-400 to-amber-500",    icon: "Award" },
  { level: 15, minXP: 31000,  label: "Staff Eng",    color: "from-lime-400 to-green-500",      icon: "Cog" },
  { level: 16, minXP: 42000,  label: "Wizard",       color: "from-emerald-400 to-teal-600",    icon: "Wand2" },
  { level: 17, minXP: 56000,  label: "Ninja",        color: "from-teal-400 to-cyan-600",       icon: "Shield" },
  { level: 18, minXP: 75000,  label: "Legend",       color: "from-sky-500 to-blue-700",        icon: "Swords" },
  { level: 19, minXP: 100000, label: "Master",       color: "from-violet-500 to-purple-700",   icon: "Gem" },
  { level: 20, minXP: 135000, label: "Guru",         color: "from-yellow-400 to-yellow-600",   icon: "Sparkles" },
];
...
// ── Accurate streak from a persisted set of active days ─────────────────────
// (computeStreak above only sees each lesson's last updatedAt, so it loses
// history. These use UserStats.activeDays — every day the user was active.)
const _DAY = 86400000;
function _dayToUTC(s) {
  const [y, m, d] = String(s).split("-").map(Number);
  return Date.UTC(y, (m || 1) - 1, d || 1);
}
function _utcToDay(ms) {
  const dt = new Date(ms);
  return `${dt.getUTCFullYear()}-${String(dt.getUTCMonth() + 1).padStart(2, "0")}-${String(dt.getUTCDate()).padStart(2, "0")}`;
}

export function addActiveDay(activeDays, day) {
  const set = new Set(activeDays || []);
  if (day) set.add(day);
  return [...set].sort();
}

export function computeStreakFromDays(activeDays, today) {
  const set = new Set(activeDays || []);
  let current = 0, cursor = null;
  if (set.has(today)) cursor = _dayToUTC(today);
  else if (set.has(_utcToDay(_dayToUTC(today) - _DAY))) cursor = _dayToUTC(today) - _DAY;
  if (cursor !== null) while (set.has(_utcToDay(cursor))) { current++; cursor -= _DAY; }

  let longest = 0, run = 0, prev = null;
  for (const d of [...set].sort()) {
    const u = _dayToUTC(d);
    run = prev !== null && u === prev + _DAY ? run + 1 : 1;
...
// Last `days` calendar dates ending at `today` (oldest first), flagged active.
export function buildHeatmap(activeDays, today, days = 91) {
  const set = new Set(activeDays || []);
  const end = _dayToUTC(today);
  const out = [];
  for (let i = days - 1; i >= 0; i--) {
    const day = _utcToDay(end - i * _DAY);
